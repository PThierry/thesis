...
%% Last update jeu. 10 avril 2014 17:04:02 CEST phil
%%
...
problématique d'interconnexions des différents réseaux véhiculaires qui
...
équivalant et exigeant de fortes contraintes de sûreté, mais également de
...
élevé. Ces réseaux, initialement complètement indépendants,
nécessitent aujourd'hui d'être interconnectés afin de permettre le transfert de données
vers des interfaces de gestion unifiées locales ou parfois distantes.
Néanmoins, les domaines de sécurité et de sûreté associés à ces différents réseaux doivent être respectés.


En terme de sûreté de fonctionnement, la problématique temps réel est au
...
cadre d'un traitement nominal.


En terme de sécurité, les données transitant
d'un domaine à l'autre soient validées au moment du changement de
domaine, et autorisées selon la politique de sécurité associée au changement
de domaine. En terme de sûreté de fonctionnement, les domaines de moindre
...
d'embarcabilité (comme par exemple le support des vibrations, de la poussière, de
...
une telle passerelle. J'ai donc cherché à proposer une solution:
\begin{itemize}
...
Ainsi, au travers de mes travaux, j'ai plus cherché à déterminer 
...
sécurité donnée. Je dénote ces modules {\it moniteurs de sécurité}. Ces moniteurs
...
validation, j'ai définit dans un premier temps un ensemble d'équations
...
le chaînage des traitements à effectuer sur chacun d'entre eux en fonction du
...
et d'une pré-évaluation au sens des Critères Communs, de niveau EAL5.
Certaines propriétés de la fonction diode dépassant le niveau 5, la
pré-évaluation a été validé pour un niveau EAL5+. Néanmoins, le moniteur
diode, tout comme les autres moniteurs, n'ont pas été validés au sens de
la sûreté de fonctionnement, au travers d'une validation SIL. Cette validation
...
d'une véritable passerelle, et en fonction de niveau de certification demandé,
il peut être nécessaire de remplacer Linux par un environnement certifiable
sachant répondre au même besoin.\\
...
mécanismes classique de durcissement des équipements militaires, au prorata
...
contraintes, qu'elles soient liées à la sécurité, la sûreté de fonctionnement,
...
Les différentes travaux que nous avons réalisé ont également permis, dans le
cadre d'un ordonnancement à criticté mixte, d'intégrer la possibilité de
...
pouvoir décoréler des familles de traitements (par exemple pour des raisons
de sécurité ou encore de sûreté de fonctionnement), restent aujourd'hui
complexes dans le cadre des architectures multi-c{\oe}urs. Les interactions et
...
Il reste donc aujourd'hui un grand nombre de sujets complexes à étudier afin
de pouvoir intégrer dans le monde industriel les derniers travaux tant en
terme de temps réel que de sécurité logicielle.
%
