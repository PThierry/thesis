...
%% Last update lun. 21 avril 2014 11:28:08 CEST phil
%%
...
beaucoup évolué ces dernières années. De tels systèmes intègre à la fois des
mécaniques de protection de l'usager tels que l'{\it Anti-Lock Breaking System}
...
activables ou desactivables par le conducteur via une interface de contrôle.
Ses sous-systèmes sont compatibles avec des exigences temps réel fortes, dûes à la
problèmatique de sûreté de fonctionnement qui leur est associée.\\
...
véhiculaire. C'est par exemple le cas des radars de reculs, parfois couplés à
...
parfois à sens unique, afin d'assurer la sûreté de fonctionnement de certains
sous-systèmes.\\
Avec l'intégration de terminaux utilisateurs dans le cadre de la gestion du
multimédia, viennent se greffer ces réseaux IP dont le cloisonnement est
aujourd'hui de plus en plus complexe à assurer. En effet, la complexité de
l'ordinateur de bord est de plus en plus grande, car ce dernier fournit de
plus en splus de service. Dans les études en cours, les constructeurs
cherchent également à interconnecter le véhicule avec des réseaux distants, et
l'utilisateur pourras accéder, via ces derniers, à diverses informations
moteurs (e.g. usure des pièces, état général du véhicule, etc.), impliquant un
...
L'ordinateur de bord, élément dont l'interface utilisateur est unifié, permet
...
différents sous-systèmes d'aide à la conduite, etc). Une telle architecture
...
\label{subsec:systronic_problematique}
...
interagir avec une base de données patients centralisée, du fait des exigences
de confidentialité associées aux données médicales.\\
Dans le cas des véhicules militaires, des exigences de sûreté de
fonctionnement sont également présentes sur la partie caméra, lorsque le
...
\begin{itemize}
  ...
  \item Le réseau critique, ayant de forte exigences de sûreté de
    ...
    Ce réseau gère les éléments critiques du véhicule tels que les caméras ou
    les systèmes de désactivation des mécanismes défensifs en cas d'ouverture
    de portes ;
...
répondre aux besoins du domaine de sûreté le plus faible. C'est un besoin
important dans le traitement de flux vidéo ou voix émis à partir d'un domaine
de sûreté élevé. Dans ces deux cas, un débit et une latence garantis sont
nécessaires, comme définit dans l'exigence
\hyperlink{REQTEMPS001}{REQ\_TEMPS\_001}. C'est également le
cas de certains flux d'alertes, comme la remonté d'alarme des différentes
sondes moteur. Cela implique également que les réseaux interconnectés soient
aptes à assurer des délais. Ainsi, sur un réseau systronique de type Ethernet,
on s'appuiera sur du réseau TT-Ethernet (Time Triggered Ethernet) \cite{steiner2009ttethernet} synchronisé
temporellement via un protocole type IEEE 1588 \cite{steiner2009ttethernet}.\\
...
sûreté plus faible ne peuvent être certifiés pour le traitement de flux
dont le niveau de sûreté est supérieur, ces derniers ne peuvent émettre de
requêtes vers un domaine de sûreté supérieur sans validation préalable par un agent de
confiance. Il peut parfois être exigé que la communication soit garantie à
sens unique entre deux domaines de sûreté hétérogènes. Dans ce dernier cas, un
...
de domaine de sûreté plus faible, mais plutôt émettre une information de
...
systèmes de niveau de sûreté et de sécurité hétérogènes avec un grand nombre
d'interactions.\\
Ces interactions nécessite d'être filtrées, validées voire inspectée afin de
garantir à la fois le besoin sécuritaire et le besoin de sûreté des différents
...
communications inter-domaines, et doit être compatible des exigences de sûreté
...
militaires, impliquent à la fois l'interconnexion de systèmes de sûreté et de
...
\paragraph{}
La conséquence de la problématique décrite en
\ref{subsec:systronic_problematique} est que
...
extérieure lors de la production serait un atout en terme de coût dans le cadre
...
sûreté de fonctionnement, comme la diode logicielle. Dans ce cas, le conteneur
...
sûreté), comme je l'ai décrit dans le Chapitre \ref{chap:solution_secu}. Dans
...
la certification des différentes fonctions, si ces dernières sont garanties
...
Comme décrit dans la Figure \ref{fig:block}, j'ai implémenté un mécanisme de
...
\ref{sec:pikeos}, il aurait alors été nécessaire de déterminer correctement à la
fois l'ordonnancement des différents conteneurs et celles des tâches dans ces
derniers. Pour répondre à cette problématique, j'ai défini dans la Table
\ref{tab:contr_hier_tdm} cinq équations permettant de déterminer si le système est
alors apte à garantir l'ordonnançabilité de l'ensemble des fonctions temps
réel, dans le cadre d'un ordonnancement de type EDF sur un partitionnement
global de type TDM.
...
remonter des alertes en cas de non-respect de la politique de filtrage ou en
...
\begin{itemize}
  \item Un récepteur de paquet, qui reçoit la donnée et valide son contenu
    ...
    paquet. Il est donc nécessaire de le mesurer soit via une étude
    statistique sur son coût d'exécution en générant un flux réseau
    représentatif, permettant d'avoir une valeur approchante mais non
    garantie, soit en s'appuyant sur une analyse statique du code.
  \item Un émetteur, récupérant les paquets tagués par le récepteur comme
    valide et qui les reexpédie vers le compartiment suivant. Le WCET de cette
    tâche est assez stable, son coût n'étant dépendant que de la taille de
    trame.
\end{itemize}
...
impliquerait entre autres un mécanisme permettant de concaténer les messages
...
plus un mécanisme de gestion de queues de messages multiples couplées à une
...
seule trame réseau. Il existe plusieurs type de contenu TLV. Bien que les
trames soient parfois de taille identique, le système DPI cherche à étudier
l'information qui y est stockée, afin de déterminer une action en
conséquence. Le filtre est donc en charge d'assurer que le contenu d'une
trame, dans le cadre d'un séquencement de trame valide (e.g. type 1, type 2,
type 2, type 5) est bien compatible avec la politique de sécurité.
...
vers le domaine pays doivent dans le même temps assurer que les données
...
\subsubsection{A propos du conteneur émetteur périodique}
...
d'assurer un débit (en nombre de trames) fixe, avec une période inter-trames
...
\begin{enumerate}
  ...
  \item Afin d'éviter tout canal caché basé sur la période inter-trame, cette
    ...
    émettre. Ce comportement permet d'être compatible avec l'Exigence 
...
\paragraph{}
La configuration de la maquette et le flux d'entrée sont décrits dans la Table \ref{tab:dut}.\\
\begin{table}
\begin{center}
\begin{tabular}{|l|l|}
  \hline
  ...
  {\it taille de payload} & type 0: 50 octets \\
   & type 1: 50 octets\\
   & type 2: 30 octets\\
   & type 3: 30 octets\\
   & type 4: 70 octets\\
...
différents conteneurs. Dans ce cadre, l'ordonnancement n'est pas de type
...
d'ordonnancement de type Rate Monotonic, avec la configuration suivante :
{\small
\begin{table}
\begin{center}
\scalebox{0.9}{
...
Chaque type possède sa propre taille et son propre contenu. Un émetteur et un
...
est droppé et un compteur de statistiques est mis à jour. Dans un cas plus
...
\paragraph{{\it Émetteur périodique}}
...
n'ont pas été définies, ces dernières étant directement dépendantes de la période
...
\begin{figure}
  \begin{minipage}[b]{.46\linewidth}
    ...
    %\scalebox{9cm}{
    %\centering\input{figures/measure_all.tex}
    %}
...
montrent le profil d'exécution des trois tâches de la passerelle dans le
...
positionnée à 1 milliseconde. Les Figures \ref{fig:filter_p_2} et
...
\paragraph{}
Nous avons vu que le connecteur ne permet pas
...
\begin{itemize}
  ...
  \item Les profondeurs de buffers doivent être déterminées en nombre de
    paquets et non en octets, afin de ne pas être impactés par la variation de
