...
\property{The average input flow throughput should be one packet per sender
  period $P_S$}\\
\property{The input flow burst size should not be greater than the
  connector's input buffer depth}\\
\property{For a given burst, the period between too packets should be fixed
  to $P_E$}\\
\property{Another burst can be sent only if the previous one has been
