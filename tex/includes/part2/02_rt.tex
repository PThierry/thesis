...
%% Last update lun. 21 avril 2014 11:24:12 CEST phil
...
mixte afin de pouvoir limiter le nombre de traitements effectué sur chaque
...
similaire aux systèmes temps réel à ordonnancement hiérarchique \cite{regehr_evolving_2003}. On
...
exécuté. L'exécution successive de l'ensemble des slots du système forme un
...
\paragraph{}
Plusieurs variables sont introduites dans la Table \ref{tab:vars} afin de formaliser l'ordonnançabilité
 ...
 \fbox{
    \begin{minipage}{0.97\linewidth}
        ...
        $\tau_{i}$ l'ensemble de tâches du compartiment $C_{i}$, $\tau_{i} = \{ \tau_i^{1},
            \ldots, \tau_i^{n} \}$\\
            $d_{i}^j$ l'échéance de la tâche $\tau_{i}^{j}$ de $C_{i}$\\
            $c_{i}^j$ le WCET de la tâche $\tau_{i}^{j}$ de $C_{i}$\\
            $t_i^j$ la période de la tâche $\tau_{i}^{j}$ de $C_{i}$\\
            \vspace{10.5mm}
    \end{minipage}
 }
 \end{minipage}
...
\fbox{
    \begin{minipage}{0.98\textwidth}
        ...
        \begin{minipage}{0.52\textwidth}
                    {\small
            \begin{equation}
                \forall i\in \{ 1, \ldots, m \}, \forall \tau_i^j \in S_i,
                d_i^j>ST_i-SC_i+c_i^j
            ...
            \begin{equation}
                            h(t) \leq t - k(ST_{i} - SC_{i})
                                \label{eqn:edf_sched}
            \end{equation}
                        {\it avec} $h(t) = \sum_{j = 1}^{n} max (0, 1 +
                        \lfloor \frac{t - d_i^{j}}{t_i^{j}} \rfloor)c_i^{j}$,
                        \end{center}
                        }
        \end{minipage}\\
        \end{tabular}
...
\end{minipage} \\
\end{tabular}
\caption{Formalisation des contraintes sur l'ordonnancement hiérarchique de
tâches en mode TDM/EDF\label{tab:contr_hier_tdm}}
\end{table}

Les contraintes globales sont les suivantes:\\
L'Equation \ref{eqn:vm_load_sum} garantit que la charge processeur est
...
\ref{eqn:slots_pgcd} est une condition suffisante pour assurer que les slots
...
période de $ST_i$ unités de temps. Les tâches exécutées dans ce slot le sont
...
\begin{itemize}
  ...
  \item sur des fonctions logicielles qui génère une surcharge non maîtrisée
    ...
    des travaux commencent à être faits pour intégrer leur impact
...
le noyau Linux. Dans le cadre de ma thèse, afin d'avoir un maquettage rapide,
...
\end{itemize}
Néanmoins, un démonstrateur plus performant aurait également pu être fait sur
une base Xenomai, en s'appuyant sur son module
rtNet \cite{tian2009implementation}. Néanmoins, afin d'intégrer un
comportement temps réel performant dans un environnement Linux virtualisé sur
le micro-noyau PikeOS, il aurait été nécessaire de porter la
solution Xenomai sur le micro-noyau PikeOS, travail qui n'a pas été effectué
dans le cadre de ma thèse.

\paragraph{}
Dans le cadre de mes travaux, j'ai dimensionné l'impact de ce patch sur la
variation de latence à l'initialisation d'un travail de priorité maximum. Cette
...
Je décris plus précisément ces différentes mesure dans le Chapitre \ref{sec:passsystronique},
...
réseaux, il est possible de classer ces différents traitements en fonction
de leur impact fonctionnel, et donc de déterminer une classement en terme
...
\begin{itemize}
	\item $r_i \in \mathbb{N}$ est l'instant de réveil du premier
        travail de la tâche $\tau_i$;
	\item $t_i \in \mathbb{N}^*$ est la période de la tâche $\tau_i$;
	\item $d_i \in \mathbb{N}^*$ est l'échéance de la tâche $\tau_i$, $d_i \leq t_i$;
	\item $\chi_i \in \mathbb{N}$ est le niveau de criticité
          maximum de la tâche $\tau_i$,
          $\chi_i \leq \mathnormal{L}$;
	\item $C_i \in \mathbb{N}^{\mathnormal{L}}$ est le vecteur $C_i = \{
        c_i(1),\ldots,c_i(\chi_i)\}$
...
\begin{itemize}
	\item $\forall m \in [1, \chi_i[$, $c_i(m) \leq c_i(m+1)$;
	\item $\forall m \in [\chi_i, \mathnormal{L}[$, $c_i(m) = c_i(\chi_i)$.
\end{itemize}
Cela suppose qu'aucune tâche ne s'exécute plus longtemps que son WCET pour son
niveau courant de criticité.
Le $k^\text{n}$-ème travail $J_k^i$ instancié par une tâche à criticité multiple $\tau_i$
est caractérisé par un 5-uplet $\{r_k^i, d_k^i, \mathcal{X}_k^i, C_k^i,
  c_k^i\}$ où:
\begin{itemize}
	\item $r_k^i \in \mathbb{N}$ est l'instant où $J_k^i$ est
          instancié.
          Étant donné que l'on considère un ensemble de tâches sporadique, on
          a $r_k^i \geq r_{k-1}^i + t_i$, et $r_1^i = r_i$;
	\item $d_k^i \in \mathbb{N}^*$ est l'échéance absolue de
          $J_k^i$. Plus précisément: $d_k^i = r_k^i + d_i$;
	\item $\mathcal{X}_k^i \in \mathbb{N}$ est la criticité de
          $J_k^i$, héritée de la tâche $\tau_i$: $\mathcal{X}_k^i = \mathcal{X}_i$;
	\item $C_k^i \in \mathbb{N}^{\mathnormal{L}}$ est la taille
          $L$ du vecteur de WCETs pour $J_k^i$, hérité de la tâche $\tau_i$: $c_k^i = c_i$;
	\item $c_k^i \in \mathbb{N}^*$ est l'exact coût d'exécution
          du travail $J_k^i$. Du fait des spécifications de $\tau_i$, on peut dire
          que $c_k^i \leq c_i(\mathcal{X}_i)$, mais la valeur exacte de
          $c_k^i$ n'est connue que lorsque $J_k^i$ termine son exécution.
\end{itemize}

\begin{definition}
Une tâche $\tau_i$ de criticité minimum $\ell+1$ est sujette à un \emph{dépassement
d'exécution de niveau $\ell$}, si un travail instancié par $\tau_i$ dépasse son
coût d'exécution pire cas de niveau $\ell$.
\end{definition}

A tout instant $t$, on appelle le $j^\text{ème}$ travail $J_j^i$ instancié par la
tâche $\tau_i$ \emph{disponible} si $t \geq r_j^i$ et $J_j^i$ n'a pas encore
terminé son exécution. Le coût d'exécution effectif du travail $J_j^i$ n'est pas
mesurable à partir des spécifications de $\tau_i$, mais ne sera connu qu'une
fois que $J_j^i$ aura terminé son exécution.
...
pour chacun des $k$ travaux instanciés par $\tau_i$ qui ont déjà terminé leur
...
faisable si tous les travaux $J_j^i \in s_i^t$, $\forall i$, avec $\chi_i \geq \ell$,
terminent leur temps d'exécution $c_j^i$ entre son instanciation et son
échéance.
\end{definition}
...
d'exécution exact des travaux qu'une fois que ces derniers ont terminé leur
exécution, le niveau de criticité du scénario ne peut être connu qu'après
coup. Les travaux instanciés sont alors ordonnancés. Dès qu'un travail dépasse son
coût d'exécution pire cas de niveau $\ell-1$, la criticité du scénario est
accrue au niveau $\ell$, les travaux instanciés de criticité inférieure à $\ell$
...
\begin{itemize}
	\item Exécutée: une tâche $\tau_i$ est exécutée lorsque
	...
	\item Prête: une tâche $\tau_i$ atteint l'état prêt si tous
	...
	\item Suspendue: une tâche $\tau_i$ atteint l'état suspendu
...
\begin{itemize}
	\item Démarre: La tâche est sélectionnée par l'ordonnanceur
          pour être exécutée.
	\item Préempte: La tâche est préemptée par une autre tâche.
	\item Termine: La tâche a complété correctement sont
          exécution.
	\item Abandonne: La criticité du système atteint un niveau
	...
	\item Active [$\ell > \chi_i$]: La tâche souhaite
          instancier
          un nouveau travail, mais le niveau courant de criticité du système est
          supérieur au niveau de criticité de la tâche.
	\item Active [$\ell \leq \chi_i$]: La tâche souhaite
          instancier
          un nouveau travail, et le niveau de criticité du système est inférieur
...
Tout scénario pour lesquels aucun travail ne dépasse un coût d'exécution de 1 est
...
temps est de criticité 2. Si tout travail de la tâche $\tau_3$ s'exécute entre 4
et 7 unités de temps, alors cela définit un scénario de criticité 3. Pour
finir, tout autre scénario est erroné.
\end{exmpl}

\begin{definition}
Le temps de réponse pire cas $\phi_i$ de la tâche $\tau_i$ est la durée maximum
pour exécuter la tâche $\tau_i$, en prenant en compte l'exécution de toutes
les tâches de priorités supérieures à $\tau_i$. On a $\phi_i \geq c_i(\chi_i)$.
\end{definition}
...
tâche $\tau_i$ se présente à chaque fois que $\tau_i$ instancie un travail de
...
que $\tau_i$ instancie un travail simultanément avec toutes les tâches $\tau_j$ de
priorité supérieure.
\end{theorem}
\begin{proof}
...
Liu \cite{Liu2000} sont réutilisables, ce qui prouve notre théorème.
\end{proof}
...
{MC-scheduling}, appliqué à un ensemble fini de travaux, est ${NP}$-complet.
...
\begin{definition}
  ...
  suivantes sont satisfaites:
	\begin{enumerate}
		...
		\item tout travail instancié par $\tau_i$ respecte son échéance
                  lorsqu'il est exécuté pour au plus $c_i(\chi_i)$ unité de
                  temps et toutes les autres tâches $\tau_j \in \tau$
                  instancient des travaux qui s'exécutent pour au plus
...
\Tq{$\tau \neq \varnothing$}{
	\uSi{aucune tâche $\tau_i \in \tau$ n'est viable à la priorité la plus basse}{
		\Retour{erreur}\;
	}
	\Sinon{
...
dit qu'un ensemble de tâches ordonnançable selon une priorité basée sur
  la criticité propre des tâches ({\it Own Criticality Based Priority} (ou
{\it OCPB-schedulability})) si on peut attribuer à chaque tâche sa priorité
fixe en utilisant l'algorithme \ref{alg:priorityAssignment}.
...
travail instancié par la tâche $\tau_i$ peut avoir $c_i(\chi_i)$ unités de temps
d'exécution entre son instanciation et son échéance dans tout scénario de
niveau de criticité $\chi_i$ ou inférieur. Mais dans n'importe lequel de ces
scénarios, chaque travail instancié par la tâche $\tau_j$ s'exécute pendant pas
...
\vartriangleright \tau_n$ implique que chaque travail instancié par la tâche
$\tau_i$ aura $c_i(\chi_i)$ unités de temps d'exécution si aucun travail instancié par une tâche 
...
premier travail de chaque tâche pour déterminer l'ordonnançabilité (dans le
cas préemptif et pour des tâches à échéances contraintes). \`A la tâche
...
$\tau_2$ peut être assignée la priorité intermédiaire parce que son temps de réponse
...
réponse ne dépasse pas $7 < d_3$ unités de temps. $\tau$ étant
...
OCBP-ordonnançables. De plus, dans la Section \ref{sec:allowanceFP}, les
...
travail dès lors que la criticité du scénario dépasse celle du travail. En
particulier, dès que le niveau de criticité du scénario atteint $\ell$, toutes
les tâches $\tau_i$ avec $\chi_i < \ell$ sont suspendues, même si le
processeur a encore suffisamment de ressources pour pouvoir continuer à exécuter
sans impacter les contraintes temporelles des travaux de criticité d'au moins
$\ell$.
Cela est dû à la condition de faisabilité lié à l'OCBP-ordonnançabilité,
présentée dans la Section~\ref{sufficientCondition} qui ne garantit plus les
contraintes temporelles.\\
Ce principe est trop restrictif et peut être amélioré sous
certaintes conditions. Dans le cadre d'un ensemble de tâches
OCBP-ordonnançables, le lemme suivant nous permet déjà de réduire l'ensemble
des tâches à suspendre.

\begin{lemma}\label{lem1}
Soit $\tau$ un ensemble de tâches à criticité mixte OCBP-ordonnançables, et
$\tau_i$ n'importe quelle tâche dans l'ordre de priorité définit par
l'algorithme \ref{alg:priorityAssignment}. Quand un travail instancié par
$\tau_i$ atteint son coût d'exécution pire cas de niveau de criticité 
$\ell \leq \mathcal{X}_i$, il n'est jamais nécessaire de suspendre les tâches
...
niveau de criticité $\ell$. $\tau$ étant OCBP-ordonnançable, chaque travail
...
Étant donné que $\chi_i > \chi_p$,  $c_p(\chi_i) = c_p(\chi_p)$, et
la condition d'OCBP-ordonnançabilité garantie que chaque travail instancié par
$\tau_i$ peut s'exécuter jusqu'à son coût d'exécution pire cas de son propre
niveau de criticité. En conséquence, seules les tâches de plus basse criticité
...
pour lesquelles les ressources CPU disponibles sont encore suffisantes pour permettre la
terminaison de ces tâches. Nous pourrions néanmoins aller encore plus
loin et autoriser l'exécution de tâches de criticité inférieure tant que les travaux que
ces dernières sont aptes à respecter leur échéance sans remettre en cause
celles des tâches plus critiques. Cela revient à
déterminer combien de temps les tâches $\tau_i$ de criticité d'au moins
$\ell+1$ peuvent supporter la suspension de tâches de niveau de criticité
...
discutèrent du concept de domaine de faisabilité qui nous permet de déterminer
l'espace des valeurs possibles pour un paramètre spécifique d'une tâche.
...
Leur travail a ciblé le calcul de la plus grande valeur qui peut être
...
Dans le contexte de la criticité mixte, la marge de tolérance de niveau $\ell$ d'une tâche $\tau_i$ dont le niveau de
criticité est d'au moins $\ell+1$ se base sur le calcul de la marge de son
...
\begin{itemize}
	\item $U_{\ell} = \sum\limits_{\tau_p \, | \, \chi_p \geq \ell} \dfrac{c_p(\ell)}{t_p}$
          est le facteur d'utilisation processeur au niveau de criticité
          $\ell$.
	\item FP$_\text{MC}$ dénote l'algorithme préemptif à
          ...
          tâches de criticité supérieure ou égale à la criticité courant du
          système.
	\item Un modèle de faute dénoté $\dfrac{k}{W}$. Cela
          correspond, pour chaque niveau de criticité $\ell \in [0, L]$,
          d'au moins $k$ tâches ($0\leq k \leq n$), au risque de dépassement
          d'exécution de niveau de criticité $\ell$ de $k$ tâches dans une
          fenêtre temporelle glissante
          de taille $W$. On définit $W\leq\min\limits_i(t_i)$ pour éviter
          à une tâche d'être sujette à de multiple dépassements de coût
	...
	\item Pour toute tâche $\tau_i$ ordonnancée avec
          FP$_\text{MC}$, et un modèle de faute $\dfrac{k}{W}$:
		\begin{itemize}
			\item $\mathrm{hp}(i)$: l'ensemble des tâches
                          $\tau_j$ ayant une priorité supérieure à $\tau_i$;
			\item $\mathrm{hp}^R(i)$: l'ensemble de travail
                          disponibles instanciés par les tâches $\tau_j \in \mathrm{hp}(i)$;
			\item $\mathrm{hp}_{k-1}(i,\ell)$: l'ensemble
                          ...
                          conditions, on choisit les tâches les plus
                          récurrentes, i.e. les $k-1$ tâches ayant les périodes
                          les plus petites;
			\item $\mathrm{lp}(i,\ell)$: l'ensemble de
                          tâches $\tau_j$ ayant une priorité inférieure à
                          $\tau_i$ et une criticité $\chi_j \geq \ell$.
			\item $\mathrm{lp}^R(i,\ell)$: l'ensemble
                          des travaux disponibles instanciés par les tâches $\tau_j \in \mathrm{lp}(i,\ell)$;
			\item L'équation suivante permet de calculer
                          la borne supérieure du temps de réponse pire cas
                          $\phi_i$ de $\tau_i$:
				\begin{equation}\label{equ:responseTime}
					\phi_i = c_i(\chi_i) +
                                        \sum\limits_{\tau_p \in
                                        \mathrm{hp}(i)} \left\lceil \dfrac{\phi_i}{t_p} \right\rceil \times c_p(\chi_i)
				\end{equation}
		\end{itemize}
\end{itemize}
...
Les priorités assignées aux tâches sont celles qui répondent au
  ...
  suivantes:
	...
	\begin{equation}\label{equ:cond2}
		\begin{split}
			...
			&\phi_j = c_j(\chi_j) + \sum\limits_{\tau_p \in
                        \mathrm{hp}(j)} \left\lceil \dfrac{\phi_j}{t_p} \right\rceil \times c_p(\chi_j) \\
			&\quad\quad\quad\quad\quad+ \sum\limits_{\tau_p \in
                        \mathrm{hp}_{k-1}(i,\ell) \cup \tau_i} \left\lceil
                        \dfrac{\phi_j}{t_p} \right\rceil \times A_i(\ell) \leq
                        d_j
		\end{split}
	\end{equation}
...
\end{theorem}
\begin{proof}
  L'équation \ref{equ:cond3} garantit que la marge de tolérance n'autorise
  ...
  tâches de criticités supérieures pour une durée limitée par la marge de
  ...
  Cette équation ne doit être vérifiée que pour les tâches ayant une priorité
...
quand toutes les tâches s'exécutent avec leur coût d'exécution pire cas de leur
...
\begin{figure}[h!]
	...
	\end{center}
\end{figure}
Etant donné que cette étude cible les ensembles de tâches $\tau$ qui sont
ordonnançables en priorité fixe selon le principe d'{\it
...
$\tau$, nous n'avons pas implémenté de principe de marge de tolérance pour
l'algorithme EDF.

%%%%%%% Allowance Domain %%%%%%%
\subsubsection{Domaine de marge sur les WCETs}\label{sec:allowanceDomain}

%%%%%%%%%%%%%%
La Section \ref{sec:allowanceFP} a décrie comment calculer une marge de
...
\begin{definition}
  ...
  On appelle domaine de faisabilité de l'espace $A(\ell)$ l'ensemble des
...
suivant:
\begin{theorem}
Un ensemble de tâches à criticité mixte $\tau$ est ordonnançable en priorité
fixe si et seulement si:
	\begin{multline}
...
\end{theorem}
%%%%%%%%%%%%%% FIXME: REPRISE
Sachant que nous souhaitons représenter le domaine de faisabilité
de l'espace $A(\ell)$, nous avons besoin d'introduire les variables de la marge
de tolérance. L'Équation \ref{eq:domainCompact} doit être étendue afin de
considérer une marge de tolérance de niveau $\ell$ pour chaque tâche de
criticité au moins égale à $\ell$.

\begin{theorem}
Soit $\tau$ un ensemble de tâches à criticité mixte. Le domaine de faisabilité
de la marge sur les WCETs de niveau $\ell$ est calculé comme suit:
	\begin{equation}
...
\begin{itemize}
	\item l'espace de tolérance $A(1)$: L'ensemble de points
	...
	\end{figure}
	\item Espace de tolérance $A(2)$: L'ensemble de points
		...
		L'espace de tolérance $A(2)$ est alors représenté par
	...
	\item Espace de tolérance $A(3)$: L'ensemble de points
...
\begin{definition}
  Soit $t_i$ l'instant de requête du $k^\text{ième}$ travail instancié par la
...
\begin{definition}
Soit $J_k^i$ le $k^\text{ième}$ travail instancié à l'instant $t_i$ par la tâche
...
$t_i$ comme suit:
	\begin{equation}
		...
		\end{split}
	\end{equation}
L'instant d'exécution au plus tard des travaux $J$ de $\mathrm{lp}^R(i,\ell)$ est
...
Basé sur le $\mathsf{LET}_i^{\ell}(t_i)$ d'un travail dont le niveau de criticité
est supérieur à $\ell$, on peut initialiser le timer en charge de détecter un
dépassement d'exécution pire cas de niveau $\ell$. Lors de la terminaison de
ce dernier, toutes les tâches de criticité inférieure à $\ell$ et non encore
terminées doivent être désactivées. Nous
n'initialisons pas de timer pour les travaux dont la criticité est égale à
...
même, pour calculer le $\mathsf{LET}$ des autres travaux.\\
...
l'instant d'excution au plus tard du premier travail de chaque tâche calculés, ces
...
augmentée à l'instant 1, empêchant la tâche $\tau_1$ de terminer son
exécution.
...
dépasse son coût d'exécution pire cas de niveau de criticité 2, mais termine
son exécution suffisamment tôt pour éviter l'interruption de la tâche
$\tau_2$, qui possède encore assez de temps pour terminer son exécution pour
...
criticité du scénario aurait été augmentée à l'instant 3, empêchant la tâche
$\tau_2$ de terminer son exécution.
...
Pour finir, la Figure \ref{ex:allowanceReuse} illustre la récaupération de la
marge de tolérance. En effet, la tâche $\tau_3$ avait une marge de tolérance de
une unité de temps, mais a pu terminer son exécution avec un coût inférieur à
son WCET du niveau de criticité courant.
Cela a permis à la tâche $\tau_2$ de profiter de deux unités de temps
...
\subsection{Réinitialiser la Criticité du système}\label{sec:resettingCriticality}
...
plus possible, empêchant de désactiver des travaux qui auraient pu avoir
suffisamment de temps pour terminer leur exécution. Néanmoins, dans certains
cas et à un niveau de criticité donné, une tâche peut nécessiter plus de temps
pour terminer son exécution que la marge de tolérance ne lui permet. Dans ce
...
suspendre des tâches de priorité inférieure pour garantir les contraintes
...
durant lequel aucun travail de criticité au moins égale à $\ell$ n'est disponible,
en attente d'ordonnancement à l'instant $t$.
\end{definition}

\begin{theorem}\label{theo:decreaseCriticality}
Soit $\tau$ un ensemble de tâches périodiques à criticité mixte
ordonnançable. Dès qu'un instant d'inactivité de niveau $\ell$ se produit
...
tous les travaux instanciés par une tâche dont le niveau de criticité est au moins
aussi élevé que que la criticité du scénario à l'instant $t_{\text{idle}}$ ont
terminé leur exécution, et qu'aucun travail n'est disponible. Soit $\tau_i$
n'importe quelle tâche de $\tau$ qui a été désactivée avant $t_{\text{idle}}$.
Nous devons montrer qu'à l'instant $t_{\text{idle}}$, tous les travaux instanciés
...
$J_i$ le premier travail instancié par $\tau_i$ à l'instant $t_i$ ($t_i \geq
t_{\text{idle}}$). On distingue deux cas:
  
 \begin{enumerate}
	\item $t_i$ est l'instant critique pour $\tau_i$: il s'en suit que le
          temps de réponse de $J_i$ sera le plus grand parmi tous les travaux
          instanciés par $\tau_i$. Mais comme $\tau$ est ordonnançable au sens
          ...
          n'instancie de travail qui s'exécute pour plus de $c_j(\chi_i)$ unités
          de temps.
	\item $t_i$ n'est pas un instant critique pour $\tau_i$: dans ce cas,
          ...
          respectera également son échéance dans un instant non-critique.
\end{enumerate}
\end{proof}
...
tâche illustré dans la Figure \ref{ex:decreaseCriticality}. Le premier travail
instancié par la tâche $\tau_3$ termine son exécution après 7 unités de
...
être réinitialisé à 1, et les travaux instanciés par $\tau_1$ et $\tau_2$ peuvent
être à nouveau considérés. La tâche $\tau_2$ instancie son travail suivant à
l'instant $t = 7$, mais il est préempté par la tâche $\tau_3$ à l'instant $t =
8$. Etant donné que $\tau_3$ a terminé son exécution, après 3 unités de temps, le
...
tâches d'instancier un travail de nouveau apte à respecter son échéance.
\begin{figure}[h!]
	\begin{center}
		\includegraphics[scale=0.5]{./figures/decreaseCriticality.png}
		\caption{Diminution de criticité}\label{ex:decreaseCriticality}
	\end{center}
\end{figure}
\end{exmpl}
...
Dans cette Section, nous comparons les performances de trois solutions possible
pour traiter l'accroissement de criticité lors de l'ordonnancement d'un ensemble
de tâches à criticité mixte:
\begin{enumerate}
	...
	\item L'approche traditionnelle, étendue avec un mécanisme de
          diminution de criticité (CD, pour {\it Criticality Decrease}):dès qu'une tâche $\tau_i$ dépasse son
...
A chaque travail $J_k^i$ instancié par une tâche $\tau_i$ est assignée une durée
$c_{i,k}^e$ dans l'intervalle $[1,c_i(\chi_i)]$, en utilisant une distribution
triangulaire. Sachant qu'un travail ne termine son exécution ni après un très
...
$\frac{c_i(\chi_i)}{3}$. Cela signifie qu'un travail va la plupart du temps
...
La durée exacte de chaque travail est complètement indépendante de la durée exacte
des travaux précédents instanciés par la même tâche. Un travail $J_k^i$ complètera
...
Nous avons comparé le nombre moyen de travaux qui ont dû être supprimés par les
...
chaque charge de tâche (on note cette borne supérieure en tant que
...
tandis que l'axe des Y représente le pourcentage moyen de travaux ayant été
supprimés.
...
\begin{table}[h!]
	\begin{center}
		\begin{tabular}{|c||c|c|c|}
			...
			\hline
		\end{tabular}\caption{Pourcentage moyen de travail supprimés en fonction de  $U_{\text{max}}(\tau_i)$}\label{fig:results}
	\end{center}
\end{table}
...
\begin{figure}[h!]
	\begin{center}
		...
		\caption{Pourcentage moyen de travaux supprimés en fonction de la charge\label{fig:resultsGraphical}}
	\end{center}
\end{figure}

Nous observons que le mécanisme de diminution de criticité peut réduire le
nombre de travaux supprimés de 25\% en pire cas, alors que cette réduction
...
tolérance, le nombre de travaux supprimés étant réduit de 5\% à 9\% de plus.\\
...
%Mcould reduce up to 24\% the number of travaux that had to be suspended, while the allowance could decrease
...
spatial et temporel autonome pour des raisons de sécurité, peuvent être ordonnancés de
...
ordonnancement mono-processeur. Dans le cadre de ma thèse, je considère que de tels ensembles de
tâches sont seuls sur le c{\oe}ur du processeur sur lequel ils sont assignés.
Néanmoins, il est possible d'étendre les résultats précédents dans le cadre
d'un ordonnancement partitionné de tâches à criticité mixte sur une architecture
multi-c{\oe}urs.
