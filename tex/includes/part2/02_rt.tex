...
\caption{Ordonnancement hierarchique sur base TDM\label{fig:hyp_global_sched}}
\end{figure}
...
\end{minipage} \\
\end{tabular}
\caption{Formalisation des contraintes sur l'ordonnancement hiérarchiques de tâches sur base TDM\label{tab:contr_hier_tdm}}
\end{table}
...
\begin{itemize}
  ...
  \item sur des fonctions logicielles qui cassent la mécanique
    d'ordonnancement via du {\it vol de cycle} : ces fonctions
    ...
    des travaux commencent à être fait sur la capacité à intégrer leur impact
...
\begin{figure}[ht]
  \begin{center}
    ...
    Linux avec PREEMPT\_RT\label{fig:rttest_1}}
  \end{center}
\end{figure}
...
souple dans ces compartiments. Les tâches à contraintes temps réel fortes ne
...
tâches nécessaire aux divers traitements.
...
l'exécution de l'ensemble de tâche où il est possible d'exécuter à nouveau
...
A tout moment $t$ on appelle le \emph{scénario} $s_i^t$ de la tâche $\tau_i$
...
\begin{definition}
Une politique d'ordonnancement en ligne est correct pour un ensemble de tâches
...
Une tâche ne peut être dans un des états suivants:
\begin{itemize}
...
de criticité 1, tandis que tout scénarios pour lequel les tâches $\tau_2$ et
...
être étendu aux ensemble de tâches à criticté mixte.
...
\caption{Algorithme d'assignation d'un ordre total aux tâches d'un ensemble à
...
Si un tel ordre de priorité existe, i.e. si une tâche viable à la priorité la
plus basse est trouvé à chaque étape (de l'ensemble restant de tâches de
priorité non assignée), alors chaque tâche $\tau_i$ est garantie de respecter
...
tâches traditionnels. Néanmoins, la proposition suivante déclare que la
stratégie EDF (Earliest Deadline First) n'est pas optimale pour les ensembles
de tâches à criticité mixte.

\begin{proposition}
Tout ensemble de tâches à criticité mixte $\tau$ qui est ordonnançable
...
d'ordonnançabilité pour les ensembles de tâches périodique. Nous avons adapté
...
duquel les tâches de criticité inférieure nécessite d'être désactivées.
...
\ref{ex:mcTaskSet1}, est il est assumé que toutes les tâches de criticité au
...
l'instant d'excution au plus tard du premier job de chaque tâche sont
...
t_{\text{idle}}$). On distingue deux cas:
 ...
 \begin{enumerate}
	...
	\item $t_i$ n'est pas un instant critique pour $\tau_i$: dans ce cas,
          ...
          au cooût d'exécution pire cas de la tâche $\tau_i$. Mais nous avons
...
des jobs précédants instanciés par la même tâche. Un job $J_k^i$ complètera
...
de tolérance plus grand peut être apportée à chaque tâche.
...
ensembles de tâches à criticité mixte, intégrés chacun dans un compartiment
...
qui est l'affinité de l'ensemble de tâche, limité pour le moment à un seul
c{\oe}ur.
