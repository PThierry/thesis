...
\begin{itemize}
	\item $r_i \in \mathbb{N}$ est le moment distanciation du premier
...
d'exécution de niveau $\ell$}, si un travail instancié par $\tau_i$ dépasse sont
...
\begin{itemize}
	\item $\tau_1: \{0, 5, 5, 1, (1,1,1)\}$;
	\item $\tau_2: \{0, 7, 7, 2, (1,3,3)\}$;
	\item $\tau_3: \{0, 8, 8, 3, (1,3,7)\}$.\\
\end{itemize}
...
\begin{definition}
Le temps de réponse pire cas $r^*_i$ de la tâche $\tau_i$ est la durée maximum
pour exécuter la tâche $\tau_i$, en prenant en compte l'exécution de toutes
les tâches de priorités supérieures à $\tau_i$. On a $r^*_i \geq c_i(\chi_i)$.
\end{definition}
		...
		\begin{itemize}
			...
			\item L'équation suivante permet de calculer
				...
				\begin{equation}\label{equ:responseTime}
					r^*_i = c_i(\chi_i) +
                                        \sum\limits_{\tau_p \in
                                        \mathrm{hp}(i)} \left\lceil \dfrac{r^*_i}{t_p} \right\rceil \times c_p(\chi_i)
				\end{equation}
		\end{itemize}
\end{itemize}
  ...
  suivantes:
	...
	\begin{equation}\label{equ:cond2}
		\begin{split}
			...
			&r^*_j = c_j(\chi_j) + \sum\limits_{\tau_p \in
                        \mathrm{hp}(j)} \left\lceil \dfrac{r^*_j}{t_p} \right\rceil \times c_p(\chi_j) \\
			&\quad\quad\quad\quad\quad+ \sum\limits_{\tau_p \in
                        \mathrm{hp}_{k-1}(i,\ell) \cup \tau_i} \left\lceil
                        \dfrac{r^*_j}{t_p} \right\rceil \times A_i(\ell) \leq
                        d_j
		\end{split}
	\end{equation}
