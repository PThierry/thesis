...
permettant de faire du traitement télécom avec valeur ajoutée ne permettent pas
de certification. De manière générale, seul l'usage d'éléments logiciels à
faible volumétrie de code permet de supporter une certification.\\

Parce que l'implémentation d'un passerelle de type télécom ne peut être certifiée en
terme de sécurité, les éléments d'implémentation de cette pile
...
des architectures MILS est respectée, comme le montre la Figure
\ref{fig:iofilter}.

\subsection{A propos des moniteurs de sécurité}

\paragraph{}
Il est montré dans le chapire précédant que la présence d'élément logiciels
non certifiable est un mal nécessaire pour permettre un certain nombre de
traitements à valeur ajoutés sur des flux télécoms. L'implémentation d'une pile
réseau certifiable étant trop couteuse, j'ai fait le choix d'intégrer de tels
logiciels, en considérant le problème de non-certification associée dans
l'architecture.
...
rester compatible des quatres exigences initiales des architecture MILS. Dans
...
est présent. L'ensemble de ses moniteurs définit l'ensemble $Sec$.
