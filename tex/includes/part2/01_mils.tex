...
\chapter{Définition d'une architecture logicielle sécurisée pour le MLS orientée traitement de flux}
\doMinitoc
...
de considérér la colocalisation de domaines de sûreté et de sécurité disjoints
sur un même matériel. Pour répondre à cette problématique, je propose
de définir une solution logicielle répondant aux besoins de MultiLevel Security
(MLS). En plus des contraintes de cloisonnements assurant une étanchéité
...
transférer de l'information entre les différents domaines de sécurité. Ce
transfert ne doit pas impacter les contraintes de sécurité de chacun des
domaines. Pour cela :
\begin{itemize}
  \item Les données transitant du niveau de sécurité le plus élevé au niveau le plus
    bas doivent être complètement contrôlées, afin de valider que ces
...
\paragraph{}
Afin d'être compatible de l'exigence de colocalisation, je m'appuie sur le
principe de système MLS.
Les mécanismes définis pour répondre aux besoins de type MLS impliquent le
...
compatible des systèmes MILS et MLS, auquel j'ajoute des modules de sécurité
complémentaire. L'ensemble est alors appelé Extended Secure Separation Kernel
(ESSK):\\
...
sécurité, à défaut d'avoir intégré un design matériel spécifique. Ces
...
\paragraph{}
La présente passerelle connecte des domaines de sécurité disjoints. Elle
...
compatibles d'une certification à haut niveau. De manière générale, seul
...
ne doivent pas accéder directement à un quelconque élément matériel.
\end{property}
...
rester compatible des quatre exigences initiales des architecture MLS. On
définit donc des éléments logiciels autonomes et certifiable portant une
fonction de sécurité. Par la suite, j'appelle ces éléments logiciels
des moniteurs de sécurité.\\
Ces derniers permettent de répondre aux différentes exigences du MLS car :
\begin{itemize}
...
\paragraph{}
Dans ce chapitre, je décris les choix structuraux que j'ai fait en terme
...
de sécurité de base permettant de respecter les exigences des architectures MLS.\\
...
\section{Définition de la réponse technique au besoin}

\subsection{Définition des données à protéger}

\paragraph{}
J'ai définit dans les Sections \ref{sec:mls_gw_principle} et
...
Security). En complément de la définition de ces principes généraux, permettant d'être
compatible du besoin MLS, il faut décrire plus en détail ceux nécessaires pour
répondre aux besoins décrits dans la problématique générale. Pour cela, je m'appuie
sur la méthode qui ont été définit initialement par
l'ANSSI\footnote{Agence Nationale pour la Securité des Systèmes
  d'Information}, dans le cadre d'EBIOS \cite{hemery2007utilisation}.
La méthode EBIOS permet de pouvoir définir un niveau de sécurisation
nécessaire et suffisant pour répondre au besoin initial.

\paragraph{}
La définition de l'architecture logicielle pour le besoin d'une passerelle
multi-domaines de sécurité implique de répondre aux interrogations suivantes:
\begin{itemize}
  ...
  \item Quels sont les attaques impactant ces protections dans le cadre de
    la solution ?
...
\paragraph{}
Les sections précédentes mettent en pratique les principes de la méthode EBIOS
...
niveaux de sécurité, et ainsi répondre à des besoins sécuritaires plus ou
moins fort. Je considère alors l'ensemble des risques sécuritaires comme
élevés et cherche à répondre à chacun d'entre eux. 
...
cette passerelle est de permettre le transfert de flux de type IP entre deux
...
\end{requirement}
La transmission de flux vers le domaine de sécurité le plus haut ne doit pas
générer de
\FIXME{terminer}
%%%%%%%%%%%%
...
capable de garantir une latence de traversée maximum ainsi qu'un débit garanti par construction.\\
...
\subsubsection{Problématique d'exploitation des canaux auxiliaires: à propos des profils de flux}
