...
supporter le partage de c{\oe}urs processeurs entre domaines de sécurité tout
en contrecarrant les attaques de type analyse de temps de réponse au travers du
...
cache en fonction des domaines de sécurité s'exécutant. On considère alors que
le nombre de partition est égal au nombre de domaines de sécurité, auquel on
ajoute une partition pour le {\it Secure Separation Kernel}.\\
Je considère que le domaine de plus faible sécurité est limité à
un et un seul compartiment, ordonnancé dans un c{\oe}ur processeur où deux
...
considéré. Je considère dans le cadre de ma thèse
...
de lignes de caches multiple de 4.
...
Afin de limiter au maximum le volume de code traitant de la configuration du
...
faire via l'usage de l'instruction {\tt mtspr}, dédiée à l'écriture dans les
...
problématique d'utilisation de canaux auxiliaires basés sur les contrôleurs de
