...
%% Last update mer. 16 avril 2014 13:09:07 CEST phil
...
\paragraph{}
{\it
Dans ce chapitre, je décrit l'architecture générale de la solution
logicielle. Les choix que j'effectue ont pour but d'assurer une compatibilité
avec les contraintes de sécurité de la problématique, tout en évitant de
devenir incompatible des contraintes temps réel, comme je le décrirai dans le
chapitre suivant.
}

\section{Proposition d'architecture et choix structuraux}

\subsection{Principe de passerelle de traitement de flux MultiLevel Security}
\label{sec:mls_gw_principle}

\paragraph{}
Dans le cadre de la problématique des systèmes systroniques, il est nécessaire
...
\begin{itemize}
  ...
  \item Le flux transitant du niveau de sécurité le plus bas au niveau de
    sécurité le plus élevé n'exige pas toujours de filtrage en terme de
    contenu (en fonction du risque sécuritaire mesuré, lié au contenu du flux).
...
respect des exigences définies dans le Chapitre \ref{sec:mils}, et
...
\subsection{Présence d'éléments non certifiables}
\label{sec:noncert}
...
l'usage d'éléments logiciels à faible volumétrie de code, de l'ordre de
quelques milliers de lignes, permet de supporter une telle certification à coût raisonnable.\\
...
\begin{itemize}
  \item deux familles logicielles $Sec$ et $Unsec$, l'une étant de confiance, l'autre non
...
\paragraph{}
Dans un premier temps, je définis pour les familles $Sec$ et $Unsec$ les
...
(Trusted Computing Base). Ces éléments logiciels doivent respecter un certain
nombre de contraintes :\\
\begin{itemize}
  \item faible volume de code (quelques milliers)
  \item faible complexité cyclomatique \cite{hansen1978measurement} (de l'ordre de 8 ou 9 au maximum)
  \item taux de couverture en tests-unitaires élevée
\end{itemize}
\end{property}
...
de la mémoire principale, dont l'accès est vérifié par la MMU.
\end{property}
...
\begin{figure}
  ...
  \caption{Filtrage des entrées/sorties par un élément logiciel de confiance\label{fig:iofilter}}
\end{figure}
...
\subsection{Moniteurs de sécurité}
...
traitements à valeur ajoutée sur des flux télécoms. En effet, une nouvelle implémentation d'une pile
réseau (e.g. pile TCP/IP, pile de communication sur réseaux CAN) certifiable étant trop
couteuse, j'ai fait le choix d'intégrer de tels logiciels, en prenant en
compte, pour certains d'entre eux, le problème de non-certification associée dans
l'architecture.
...
Dans le cadre de ma solution, je définis donc des éléments logiciels autonomes
...
Aux propriétés du MLS décrites dans le Chapitre \ref{subsec:mls}, que le
logiciel doit supporter, il faut également intégrer des exigences complémentaires liées
...
d'adressage IP est problématique pour le respect de l'Éxigence
...
\begin{figure}
  ...
  \caption{Exemple de séparation protocolaire couche transport\label{fig:sep_proto}}
\end{figure}
...
\begin{enumerate}
  \item Les attaques externes initiées du coté du domaine de plus haute
    sécurité. Je considère alors qu'un élément de ce domaine cherche à faire
    sortir de l'information non autorisée vers le domaine de plus faible
    sécurité, avec ou sans l'aide d'un élément de ce dernier. Il y a alors
    un risque de confidentialité du domaine Haut vers le domaine Bas.
  \item Les attaques externes initiées du coté du domaine de plus faible
    sécurité. Je considère qu'un élément du domaine de plus faible sécurité
    cherche à récupérer de l'information sur le domaine de plus haute
    sécurité, sans l'aide d'un élément de ce dernier. Il y a alors un risque
    de confidentialité du domaine Bas vers le domaine Haut.
  \item Les attaques internes, venant des éléments de la passerelle
    ...
    \ref{sec:noncert}, impactantes pour l'exigence \ref{req:escape}. Il y a
    lors un risque en terme de confidentialité, d'intégrité et de
    disponibilité.
\end{enumerate}
...
\begin{figure}
  ...
  \caption{Répartition des surfaces d'attaque sur la passerelle de sécurité\label{fig:threats_surface}}
\end{figure}
...
\begin{table}[h!]
  \begin{tabular}{|p{2.1cm}|p{5cm}|c|c|c|c|}
  \hline
\cellcolor{blue!25}Source & \cellcolor{blue!25}Attaque & \multicolumn{4}{|c|}{\cellcolor{blue!25}Exigence} \\
  \hline
  \multicolumn{2}{|c|}{ } & \ref{req:transit_up} & \ref{req:desensibilisation} & \ref{req:integrity} & \ref{req:escape} \\ 
  \hline
  {\it Domaine inférieur} & Étude temporelle de la vitesse de transit du flux
  vers le domaine supérieur & X & & & \\
  \hline
  {\it Domaine supérieur} & Émission de flux non conforme de la configuration
  de sécurité & & X & & \\
  \hline
  {\it Domaine supérieur} & Transfert de donnée par canal caché via variation
  du profil de flux & & X & & \\
  \hline
  ...
  \hline
  {\it Interne} & Écoute par canal auxiliaire d'un flux transitant par un
  autre compartiment pour retransmission & X & & & X \\
  \hline
  {\it Interne} & Communication entre deux compartiments au travers d'un canal
  caché & X & & & X \\
  \hline
\end{tabular}
\caption{Profils d'attaques et impact sur les exigences sécuritaires\label{tab:threats}}
\end{table}
...
Les deux moniteurs se partagent alors cette information au travers d'un canal
de communication sécurisé, afin que le second moniteur
...
dans la mémoire partagée. En conséquence, si les flux validés en terme
...
Bien que ce moniteur de sécurité n'aie pas été validé au travers d'une
certification dans le cadre de ma thèse,
...
distinct, comme cela se fait dans l'état de l'art \cite{sstic2006diode,tsf101}.\\
...
Sécurité de Premier Niveau (CSPN), suivi d'une pré-évaluation du micro-noyau
PikeOS sur lequel il s'exécute, au sens des Critères Communs, de niveau EAL5+. De plus, ce moniteur de sécurité est
...
mais la majorité des éléments techniques est aujourd'hui maintenue secrète par
...
\subsubsection{Réponse à la problématique des canaux cachés}
...
\begin{itemize}
  \item L'émetteur et le récepteur connaissent le protocole de transmission
    ...
    s'agit du suivant:
    \begin{itemize}
      \item Une durée inter-trame de 500 millisecondes correspond à la valeur
        \emph{0}.
      \item Une durée inter-trame de 1000 millisecondes correspond à la valeur
        \emph{1}.
    \end{itemize}
  ...
  \item Le récepteur reçoit les paquets et calcule la durée inter-trame. Il
    détermine si la période est proche des 500 millisecondes (la donnée est
...
moniteur de sécurité appelé émetteur périodique. Ce moniteur de sécurité
s'appuie sur des buffers de type FIFO (First In First Out) de profondeur fixe.
Chaque case de ces FIFO est dénoté par la suite {\it cellule}. L'automate à état de ce
...
dernier doit en effet être dimensionné afin de rester compatible avec la
capacité d'émission de l'émetteur périodique. A défaut, l'émetteur périodique
peut être dans l'obligation de supprimer certains paquets en cas de surcharge
en entrée. Les services en
charge de la transmission de ce flux doivent donc gérer la perte de paquets.\\
Ces services doivent de plus supporter la possibilité de
...
certain nombre de flux de signalisation à faible débit utilisés dans les systèmes systroniques entre plusieurs
...
entre deux domaines de sécurité pour un certain nombre de flux spécifiques à la
systronique.
...
complexe est sujet de nombreux travaux qui lui sont dédiés encore aujourd'hui.
...
débrayant les mémoires caches partagées entre les c{\oe}urs. Selon l'architecture
matérielle, la désactivation des mémoires caches partagées est plus ou moins impactante
sur les performances du logiciel, au point de rendre ce dernier complètement
inefficace. Dans ce cas, l'usage d'une architecture multi-CPU en lieu et
...
Lors du changement de domaine, les mémoires caches sont alors flushées afin de ne pas
...
contrôleur de cache. La proposition cible exclusivement le contrôleur de cache
L1 data. Le cache L1 code et le TLB (Translation Lookaside
...
nombre maximum de slots limité (égal à huit partitions dans mon étude). Chaque
...
Je prends comme hypothèse que la politique de substitution des lignes de caches de chaque
...
\begin{itemize}
  \item La routine est positionnée dans un espace mémoire non-cachable, car ne
  doit pas impacter l'état du contrôleur de cache
...
\paragraph{}
Dans cette Section, je décris une proposition d'interface matérielle
...
\begin{itemize}
  \item La taille d'un slot (en nombre de lignes de cache) doit être
    compatible avec l'algorithme de substitution du contrôleur de cache (e.g. pseudo-LRU
    4-ways).
...
le TLB. De plus, le dimensionnement des partitions doit être fait en
s'assurant que la consommation en terme de lignes de caches par chacun des
domaines de sécurité soit telle que le nombre de lignes de caches qui lui est
allouée est suffisante dans le cadre de son usage. Dans le cas inverse, un tel
partitionnement augmenterait fortement le nombre de cache-miss, et aurait un
impact plus négatif que positif. Ce dimensionnement n'est pas étudié dans le
cadre de ma thèse. De plus, si la taille de chaque partition du cache,
nécessaire pour éviter un accroissement des cache-miss, est telle qu'il n'est
pas possible de déterminer une découpe valide du contrôleur de cache L1, la
solution ne peut plus être choisie pour répondre au besoin initial de
cloisonnement du contrôleur sans flush explicite.
Pour terminer, la problématique des caches L2 et plus globalement des caches
