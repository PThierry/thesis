...
\chapter{Rappel général sur les éléments logiciels et matériels impactant}
\doMinitoc

\section{Rappel général sur les éléments matériels impactant}

\subsection{Architecture générale simplifiée}
...
L'élément central est le  bus d'intercommunication,  dont  l'accès  est  contrôlé  par  le  {\it  contrôleur  de  bus}.\\
C'est  ce  dernier  qui  permet  la  communication  entre  les  différents
organes   matériel.\\
Selon la complexité de l'architecture matérielle, le bus peut être substitué
par un interconnect ou un Network On Chip (NOC). Dans ce dernier cas, la
parallélisation des accès permet de réduire les latences d'accès par les
initiateurs (éléments matériels initiant des requêtes sur le bus
d'intercommunication). Ces derniers sont par exemple le processeur ou encore le
contrôleur DMA.

\subsection{Gestion de l'adressage mémoire centrale}

\paragraph{}
Les processeurs modernes sont équipés d'une \index{MMU}\gls{mmu}  ({\it  Memory  Management  Unit}),
voire, selon les familles de processeurs, d'une \index{I/OMMU}I/OMMU
(dénomination Intel) ou SMMU (dénomination ARM).
Ces deux éléments jouent un rôle de filtre permettant  de  fournir  des  capacités  de  sécurisation
des accès mémoire via la segrégation des accès en fonction des processus
logiciels (MMU) et en fonctions de l'initiateur (I/OMMU - SMMU). La MMU est en
coupure entre le coeur processeur et le bus, l'I/OMMU (dans le cas d'Intel) en coupure entre les
périphériques initiateurs et la mémoire.
...
\caption{Translation d'adresse mémoire, vue générale}
\end{figure}

\paragraph{}
À l'inverse des éléments logiciels exécuté dans les coeurs processeur, les éléments
matériels (hors coeur processeur) utilisent  tous  un  adressage  physique.   Ainsi,  une  recopie
\index{DMA}DMA  se  fait   directement   vers   une   adresse   physique   en
mémoire   centrale. Les éléments matériels de filtrage comme l'I/OMMU ou la
SMMU sont là pour autoriser ou interdire l'accès à un sous ensemble de la
mémoire centrale.

\paragraph{}
La MMU gère une table de correspondance entre adressage physique et adressage
logique pour chaque processus. C'est le noyau du système d'exploitation qui
initie ces tables et fournit le pointeur à la MMU. Ces dernières sont donc en
mémoire centrale. Afin d'optimiser les accès et réduire les accès mémoire, la
MMU s'appuie sur un cache local, de manière similaire aux caches de données du
processeur. Ce dernier est nommé \index{TLB}TLB ({\it Translation Lookaside
Buffer}).
...
\paragraph{}
DMA signifie Direct Memory Access. Il s'agit de contrôleurs spécialisés dans
les recopies mémoire. Il permette de gérer les recopies entre mémoire centrale
et mémoires périphériques (e.g. mémoire interne du contrôleur ethernet) sans
recours au processeur. Ainsi, la recopie se fait en "tache de fond",
permettant au processeur de continuer ses traitements après initialisation du
transfer. L'avantage principal est donc la réduction de la charge processeur.
L'inconvénient principal est l'impact en terme de temps réel et de sécurité.
En effet, le contrôleur DMA perturbe les accès au bus d'interconnexion
lorsqu'il exécute des recopies mémoire, générant de potentielles augmentation
de latence d'accès au bus de la part du processeur.
Mono ou multi canaux, ils supportent potentiellement plusieurs interfaces de
traitement et donc plusieurs élément logiciels initiateurs du traitement.

\subsection{Les transferts de données}

\paragraph{}
Les transferts de données sont par nature asynchrones. En effet, lorsqu'un
service logiciel configure le contrôleur DMA afin d'opérer une recopie
mémoire, cette dernière n'est pas synchrone de la requête. La fin de la
recopie est en générale spécifiée par une interruption.\\
...
\paragraph{}
Les arbitreurs de bus gèrent les accès concurrent au bus d'intercommunication.
Lorsque ce dernier ne permet pas de paralléliser les requêtes des initiateurs,
ces dernières sont traitées en série. L'arbitreur permet de gérer la file
d'attente des requêtes et peut, selon le type de bus, supporter de la qualité
de service afin d'assurer des bornes supérieures d'accès au bus. Les bus
d'intercommunications complexes fournissent une interface programmative
permettant au noyau de spécifier la qualité de service qu'il souhaite en terme
par exemple de priorisation des accès selon l'initiateur.

\paragraph{}
Dans le cadre des Network On Chip, le bus est décomposé en routeurs sur un ou
plusieurs niveaux. Ces derniers gère en interne les files d'attente des
requêtes entre initiateurs et cibles.

\section{Les interruptions}

\paragraph{}
Les interruptions matérielles sont des évènements asynchrones émis  par  le  matériel  pour  générer
l'exécution d'un élément logiciel. Il peut s'agir d'un timer (un ordonnancement
...
cycle d'exécution en cours, pour exécuter un {\it handler  d'IRQ},  que  le  noyau  à  préalablement
...
\begin{itemize}
\item Elles ne sont pas  prédictibles  car  sont  la  conséquence  d'un
évènement  asynchrone  lié  à l'environnement,  comme  un  signal   de   remplissage   de   la   mémoire   interne   d'une   carte
...
des lignes interruptions peut empêcher leur exécution (hors \index{NMI}NMI).
\item Sur certains contrôleurs d'interruption, il est possible de prioriser
ces dernières. Ainsi, une interruption peut préempter une autre interruption
moins prioritaire.
\end{itemize}

\paragraph{}
L'exécution des handlers d'interruptions se faisant directement par le processeur sans  en  informer
l'ordonnanceur logiciel  font   de    ces    dernières    des    {\it
voleurs    de    cycles}. Elles doivent donc impérativement être maîtrisée
pour permettre le support d'exigences temps réel.

\paragraph{}
Comme le montre la Figure \ref{fig:pic_cascade}, il peut y avoir  un  grand  nombre  d'interruptions
matérielles différentes. Aujourd'hui, elles ont tendance à diminuer au profit
de contrôleurs DMA chaînant les copies mémoire et du polling (i.e vérification
active par le logiciel d'informations asynchrones).
...
\subsection{La prédiction de branches}
