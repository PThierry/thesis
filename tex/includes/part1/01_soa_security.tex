...
éléments physiques disjoints. Ces dernier assurent un transfert en sens unique de données
...
sécurité est double:
\begin{itemize}
  \item Elle assure la transfert de données en sens unique. Aucune donnée du domaine de
...
simples, de l'architecture réseau de chacun deux domaines de sécurité, elle s'appuie sur deux
{\it guichets}, comme le décrit la Figure \ref{fig:diodephy} :
\begin{itemize}
  \item Un {\it guichet bas} en charge de récupérer les
    données potentiellement via une communication en mode connecté avec le
    domaine de plus base sécurité pour les transférer à la fonction diode via
  ...
  \item Un {\it guichet haut}, en charge de récupérer les
    ...
    d'une connexion potentiellement en mode connecté avec le domaine de plus haute sécurité
\end{itemize}
...
\paragraph{}
De plus, l'architecture d'une diode matérielle, telle que formalisée par Philippe
...
le transfert de données, ainsi que pour valider la réception de ses dernières.
Une telle architecture permet de garantir l'imputabilité du transfert de données
en cas d'usage invalide de la passerelle.
...
cela, elle s'appuie sur un critère de filtrage prédéfinit et non modifiable
...
différents. Il s'agit d'un équipement impliquant un facteur de forme et des
...
\section{A propos des architectures MLS (MultiLevel Security) et MILS
...
le cloisonnement de ces flux via un mécanisme évaluable au sens des Critères
Commun pour un coût raisonnable.
...
le noyau est lui-même trop complexe. L'auteur décrit alors le principe de
...
dont le cloisonnement et assuré par le noyau. L'auteur montre que le
...
architecture distribuée de sous-systèmes communiquent de manière sécurisée en
assurant que l'unique moyen de passage est maîtrisé par un élément de
confiance permettant la transmission de données noircies. Ce principe est
...
LaPadula.\\
Les systèmes MILS sont compatibles avec quatre propriétés éditées par le
...
L'évaluation de ces composant doit être suffisante pour assurer le niveau de
...
infrastructure compatible MILS. Dans \cite{milssecgrid}, les auteurs
...
\paragraph{}
Parmi les mécanismes de séparation des domaines tels que décris par la NSA
...
les solutions existantes de virtualisation et les fonctions de sécurité
qu'elles supportent.
