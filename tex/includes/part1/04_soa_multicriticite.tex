...
\paragraph{}
{\it L'usage d'un
{\it Separation Kernel} est une des solutions logicielles proposées pour répondre aux
...
possible de pouvoir ordonnancer en concurrence deux jeu de tâches en utilisant
...
implémentée dans les noyau Linux jusqu'à la version 2.6.32. Le but de cette
...
c'est à dire les traitements déportés dans le temps de certaines fonctions.
...
contrôle, ce dernier peut alors informer l'hypervieur afin de modifier
...
qui se veut temps réel, que les développeur ont appelé S-EDF, a été faite afin d'assurer
...
indépendantes à deadline contraintes.
...
même moment, Baruah et al. \cite{RBaruah2011a} ont formalisé l'analyse de
...
augmenté. Néanmoins, les systèmes à criticité mixte ne permettent pas
de diminuer le niveau de criticité d'un jeu de tâche tout en assurant
le maintien de l'ordonnançabilité de l'ensemble de tâches. De plus, les
systèmes à criticité mixte sont considérés dans le cadre d'une exécution d'un
jeu de tâche sur un seul c{\oe}ur processeur.\\
...
tâches à criticité mixte de la passerelle est ordonnancé sur un seul c{\oe}ur
processeur et qu'il n'y a pas de partage de ressources entre des tâches de
niveaux de criticité différents. L'ensemble de ces tâches font de plus partie
d'un même jeu de tâches.
Dans le Chapitre \ref{sec:mixedcrit}, je propose néanmoins une solution, dans
laquelle nous avons cherché à déterminer un instant ou il est possible de
réduire le niveau de criticité d'un jeu de tâches. Il devient alors possible
de redescendre dans un état de criticité plus faible, et donc de relancer
l'exécution de tâches qui avait été suspendues.
