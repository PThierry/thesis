...
%% Last update ven. 18 avril 2014 17:27:04 CEST phil
...
\begin{itemize}
  ...
  \item de la définition d'algorithmes de chiffrements spécifiques et de
    solutions implémentant ces algorithmes ;
  \item d'une définition d'un processus de gestion du domaine (protection
  physique et logique) ;
...
\paragraph{}
La problématique de passage d'un domaine rouge à un domaine noir est appelée
problématique rouge/noir. On la retrouve principalement dans les réseaux radio
militaires, mais elle reste un principe général applicable à toute interconnexion
...
On appelle besoin d'en connaître l'ensemble de connaissances minimum suffisant
...
uniquement les informations nécessaires à son travail, appelées droit d'en
...
des différents éléments d'un processeur \cite{germain}\cite{sidechancube} ou via l'étude de
...
Un exemple typique est de s'appuyer sur la durée intertrame minimale d'un flux
réseau donné pour fournir une information \cite{berk2005detection}. En faisant
varier la durée intertrame, il
...
\paragraph{}
Lorsqu'un transfert de données non autorisé a lieu entre deux domaines de
...
initialement une Cible de Sécurité (Security Target).

\PhDdefinition{st}{Security Target}{
  On appelle Security Target (ST, CdS en français) le document qui définit de
manière complète et rigoureuse le problème sécuritaire associant un produit.
}

A partir de la Cible de Sécurité, on définit une cible d'évaluation
(couramment appelée Target Of Evaluation), ainsi que le niveau d'assurance à
atteindre.

\PhDdefinition{toe}{Target Of Evaluation}{
  On appelle Target Of Evaluation (TOE) l'ensemble des éléments logiciels et
  matériels qui seront évalués dans le cadre d'une certification, en fonction
  du niveau d'assurance attendu, au sens des Critères Communs.
}
...
\PhDdefinition{tcb}{Trusted Computing Base}{
  ...
  et matériels de la solution cible qui sont considérés comme de confiance et
  dont la corruption génère une faille dans les propriétés de sécurité de la solution.
}
...
technologies de virtualisation), la solution peut être compatible avec le principe
de certification composite, permettant une certification par delta.

\PhDdefinition{certcomp}{Certification composite}{
On appelle certification par composite un principe permettant, lorsque la
TCB est non modifiée, de s'appuyer sur les précédentes certifications
afin de refaire une certification de l'ensemble à moindre coût
Cela permet de faire d'incrémenter la certification d'un produit.
}
...
des travaux présents et de leurs paramètres (par exemple en fonction de leurs échéances
absolues).
...
de bus, WCET trop grand ou trop petit, etc.).
...
liée à une gestion du temps gérée en interne par l'ordonnanceur (time-driven) ou
...
\begin{figure}
  ...
  \caption{Architecture logicielle générique des solutions de virtualisation de type I\label{fig:vmtype1}}
\end{figure}
...
\begin{figure}
  ...
  \caption{Architecture logicielle d'une solution de virtualisation type II\label{fig:vmtype2}}
\end{figure}
...
\begin{figure}
  ...
  \caption{Architecture logicielle d'une solution de virtualisation applicative\label{fig:vmlxc}}
\end{figure}
...
Ainsi, il se voit seul en mémoire et toute tentative d'accès à la mémoire qui
ne lui est pas officiellement fournie sera bloquée par la MMU.
En effet, pour  qu'un processus y accède, une association entre adresse physique et adresse virtuelle, locale à cet espace
...
\caption{Principe général du mapping de page mémoire dans les espaces d'adressage des processus\label{fig:mmu}}
\end{figure}
...
\end{tabular}
\end{center}
\caption{Temps d'accès mémoire sur un Intel Core 2 duo\label{tab:cache_access_time}}
\end{table}
...
centrale renvoie la donnée, qui est mise en cache en même temps qu'elle est renvoyée au processeur.
\end{itemize}

Cette architecture provoque des cache-miss plus longs (vérification par le cache avant  de  faire  la
requête au contrôleur de bus), mais permet d'utiliser le cache indépendamment
des autres c{\oe}urs du processeur (pour un cache local).
...
utilisé le moins fréquemment. En cas d'égalité de fréquence, l'algorithme LFU est utilisé pour départager
les cellules.
...
En effet, la modification de l'état des caches du processeur est induite par le
...
\begin{itemize}
  ...
  \item Calcul du nombre pire cas de cache-miss au travers de l'étude du chemin
...
d'exécution de la tâche courante, le taux de cache-miss associé. Le but est
...
intéressantes, dans le cadre des architectures QorIQ de Freescale
\cite{QorIQP4080} :
\begin{itemize}
