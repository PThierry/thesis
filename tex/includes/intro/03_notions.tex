...
important de les cloisonner dans des environnement logiques séparés, ceci
...
utilisent encore beaucoup des architectures avec caches désactivés.
...
comme le PowerQuicc de Freescale \cite{plru}. Le schéma \ref{fig:cache_plru_e300} est tiré du manuel
...
\paragraph{}
La compatibilité des différents algorithmes de cache vu ici est étudié dans le Chapitre \ref{sec:soacache_wcet}.
Dans le cadre de ma thèse, je m'appuie sur du matériel existant, de type
PowerPC pour des raison de certifiabilité. En effet, les
architecture PowerPC sont celles ayant le moins de bugs matériels référencés
parmi les grandes familles (ARM, PowerPC, x86 et Sparc). Elles ont comme bonne
propriété d'allier une consommation raisonnable à une de bonnes performances.
Aujourd'hui, les contrôleurs de caches utilisés par les c{\oe}urs PowerQuicc
sont de type Pseudo-LRU, comme vu plus haut. Les c{\oe}urs e500mc, succédants
au c{\oe}urs e300 des PowerQuicc 2, fournissent de plus plusieurs propriétés
intéressantes, dans les cadre des archtiectures QorIQ de Freescale
\cite{QorIQP4080}:
\begin{itemize}
...
et pour des raisons de confidentialité liés au travaux de Thales autour de
leur usage, ne sont pas étudié dans ma thèse.
