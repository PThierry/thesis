...
\paragraph{}
Le principe d'un canal auxiliaire consiste à s'appuyer sur une étude
...
pour lesquelles il n'est pas initialement prévu. L'usage d'un canal caché
...
initialement une cible d'évaluation (couramment appelée {\it TOE}, pour Target Of Evaluation)
\cite{cnssi_4009}, intégrant ce qui, dans le
...
\paragraph{}
Associé à la TOE, on définit le socle de confiance (couramment appelé {\it Trusted Computing Base} (TCB) \cite{cnssi_4009}. Cette
...
sécurité.\\
Les interactions à l'intérieur d'un domaine ne sont pas soumises à des exigences fortes de
...
faire transiter. L'action de noircir consiste à assurer la
confidentialité des données de diverses manières afin d'assurer que les
...
potentiellement une modification du profil de flux, ceci afin de limiter la
...
fortes car ces dernières peuvent s'exécuter sans que le moniteur de machine
...
Cette architecture place le cache et la mémoire centrale côte à côte.  Ainsi,
...
multi-c{\oe}urs ou multi-processeurs, au travers de l'usage des {\it
\index{Memory barrier}memory barriers}. Ces dernières ont pour but de
...
\paragraph{}
L'impact des caches processeurs est non-nul sur la sécurisation d'un systèmes lorsque deux domaines de
...
canal auxiliaire entre ses dernières, comme décrit plus loin.
...
de la tâche cible, afin d'associer l'arbre d'exécution détecté à une
succession algorithmes connus. Cette hypothèse est aujourd'hui plus réaliste dans le monde
...
par une tâche concurrente, grâce à l'étude de l'arbre d'exécution détecté.
...
bornée dans le temps, et garanti le contenu des cellules du cache lorsque le compartiment
...
\begin{itemize}
  ...
  \item Caclul des pire cas de cache-miss au travers de l'étude de l'arbre
    d'exécution des différentes tâches en exécution concurrente sur un cache
    sans propriétés d'aide au temps réel
\end{itemize}
...
que le maintient de certaines lignes de caches dans le contrôleur pouvait être
...
Le TLB est le cache spécialisé permettant d'optimiser la translation
...
translation sont gérée en mémoire centrale, induisant des accès mémoire
...
une mémoire les translations d'adresses afin d'éviter une
...
être considéré dans le cadre de la gestion de tâches avec des contraintes temps réel dur.
