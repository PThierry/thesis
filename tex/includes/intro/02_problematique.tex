...
\begin{definition}
L'intégrité est une propriété d'un objet à ne pas avoir été modifié par
...
sous forme d'une lame format 1U ou supérieur. De telles solutions ne
...
multimédia fournis par le véhicules nécessitent d'être protégés d'éléments
...
véhicule), semi-locaux (i.e.
entre des véhicules d'une même patrouille) et
...
données. Ces terminaux doivent être compatible de contraintes physiques fortes (poids, volume, consommation
électrique) et de contraintes de certification fortes (principalement liées au temps-réel
mais aussi, par exemple, à résistance aux vibrations dans les véhicules). Le
coût associé à la production de tels produits est donc élevé. Ces derniers ne
doivent alors pas être multipliés pour des raison de maîtrise des coûts. 
