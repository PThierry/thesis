...
\paragraph{}
Le client étatique français (Direction Générale de l'Armement)
demande aujourd'hui d'interconnecter divers réseaux
embarqués dans un véhicule. Dans le cadre des véhicules militaires, ces
réseaux peuvent transmettre des informations ayant des contraintes de temps réel, de fiabilité mais
...
véhicule) ou la police (accès aux différentes bases de données). Dans le même
...
fiabilité ou d'un domaine à forte contrainte de sécurité.
...
des enjeux, ainsi que les premiers éléments de solutions, sont détaillés dans
...
\PhDdefinition{sysdesys}{Système de systèmes}{
  ...
  conséquence d'un besoin de supervision centralisée.
}
...
grand nombre de liens d'interconnexion), en charge de gérer les composants
moteur ;
...
compatible avec un facteur de forme spécifique.
...
\noindent Dans le cadre de ma thèse, je cible les problématiques d'interconnexions
...
souvent vérifiée par des algorithmes de hachage ou des sommes de contrôle
(par exemple le CRC32 des trames réseau Ethernet ou encore les algoritmes SHA1
ou MD5 pour des besoins plutôt sécuritaires). On s'appuie également sur
...
politique de sécurité, en charge de préciser l'ensemble des fonctions de
...
international : les Critères Communs (CC) pour l'évaluation sécuritaire des
technologies de l'information \cite{cc_docs}. Ceux-ci définissent 7 niveaux
d'assurance (confer Définition \ref{def:nivass}), appelés EAL
...
place de fonctions de sécurité (comme par exemple un pare-feu de confiance) permet de
...
On appelle niveau d'assurance le niveau de certification exigé sur l'ensemble
comprenant à la fois les différents composants logiciels et matériels. Il peut
s'agir d'une certifiaction en terme de fiabilité ou de temps réel (e.g.
certification SIL) ou en terme de sécurité (via les Critères Communs). Dans le cadre des systèmes véhiculaires, on s'appuie
sur la norme SIL (Safety Integrity Level) \cite{smith2010safety}, graduée de 1 (niveau le plus faible)
...
\PhDdefinition{syssur}{Système sûr}{
  Un système est dit sûr si son niveau d'assurance est compatible avec son niveau
  de criticité. Il possède donc des
  propriétés de fiabilité et des propriétés de temps réel adéquates pour assurer
...
\item une période, déterminant l'inter-arrivée minimum entre deux travaux de la
  tâche ;
\item une échéance, qui peut être:
  \begin{itemize}
    \item relative : la durée maximum entre l'instanciation du
  travail et la terminaison de son exécution
    \item absolue : l'instant avant lequel la tâche doit s'être complètement
    exécutée
  \end{itemize}
\end{itemize}
Un système logiciel temps réel est en charge d'assurer que l'ensemble des
travaux terminent leur exécution avant leur échéance de terminaison au plus
...
temps réel et de fiabilité, sans considération particulière en terme de sécurité.
...
et/ou intérieures ou du système d'armes si ce dernier existe.
...
garanti. Ce cloisonnement est exigé pour des raisons de sûreté afin que le
...
mécanismes permettant d'interconnecter ceux-ci de manière sécurisée, afin de
...
mécanismes soient logiques et non physiques.\\
...
coupure \cite{tsf101}. Ces solutions restent aujourd'hui difficiles à intégrer
...
sous forme d'une lame format 1U ou supérieur (format classique des {\it
...
\paragraph{}
De manière générale, on définit les propriétés de sécurité d'un domaine de sécurité
...
domaines de sécurité, dont le nombre varie selon le véhicule et la
mission.
\paragraph{}
...
désactivation du système d'armes lors de l'ouverture d'une porte extérieure.\\
...
mais aussi la puissance des éléments matériels embarqués sont impactés.
La puissance nécessaire au respect strict des exigences temps réel
peut être alors incompatible avec des contraintes physiques liées au véhicule (volume,
...
latence d'exécution doit être à tout prix maintenue (système d'armes, blindage
transparent) ;
...
forte priorité génère une charge supérieure à celle qui lui est initialement
allouée. Le système n'étant plus capable d'exécuter les deux
familles conjointement, il devient nécessaire de modifier
son comportement afin de garantir malgré tout l'échéance des travaux les
...
\begin{itemize}
  \item fortement temps réel (devant garantir l'échéance de l'ensemble des
  tâches) ;
  \item temps réel souple (pouvant autoriser des dépassements d'échéance si
  ceux-ci sont ni trop nombreux ni consécutifs) ;
  \item non temps réel (sans principe d'échéance).
\end{itemize}
...
système sur les vies humaines. Ainsi, il est impératif que le système d'armes
soit fiable.
...
\begin{itemize}
  \item cloisonner de manière efficace les domaines de sécurité et de sûreté ;
...
données. Ces terminaux doivent être compatibles avec des contraintes physiques fortes (poids, volume, consommation
électrique) et avec des contraintes de certification fortes (principalement liées au temps réel
...
alors au logiciel d'assurer cette séparation. Il lui incombe également de fournir
...
système d'armes au système de communication.\\
...
\begin{itemize}
  ...
  \item ils ne sont pas certifiables pour les systèmes contraints avec
    des exigences physiques comme la résistance aux vibrations ;
...
\begin{itemize}
  \item dans les systèmes de communication vehiculaires, la cryptographie
    ...
    De plus, elle est souvent associée à des mécanismes d'évasion de fréquence afin
  ...
  \item pour la gestion des domaines de sécurité locaux, les chiffreurs utilisés
    ...
    en énergie. Ils doivent alors être substitués par un algorithme de
  ...
  \item les mécanismes d'interconnexion assurant à la fois des exigences de
    ...
    besoin, tant dans le domaine véhiculaire que dans celui de l'avionique.
\end{itemize}
...
\begin{itemize}
  ...
  \item physique, afin de garantir que le logiciel est suffisamment peu coûteux
...
travers de la conformité à chacune des exigences de cette matrice qu'est
décrite ma solution.
...
fortement critiques et des tâches plus faiblement critiques, il faut prévoir,
dans le cadre de l'intégration de l'ensemble des tâches, de potentiels
débordements de la part des tâches très critiques. On s'appuie sur un ordonnancement à
...
domaine). Afin de valider la preuve de concept, une CSPN (Cible de Sécurité de
Premier Niveau) a été faite, associée à un dépôt de
brevet \cite{thierry2013daudit}\cite{thierry2013dctrl}, suivie d'une
pré-évaluation EAL5+ du micro-noyau sous-jacent.
