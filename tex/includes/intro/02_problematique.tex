...
\subsection{Concepts d'architecture}

\paragraph{}
Dans le cadre des architecture véhiculaire, il existes un grand nombre de
systèmes.
\begin{definition}
  On appelle système un ensemble d'éléments matériels et logiciels
  interagissant ensemble dans le but d'effectuer une fonction ou un ensembles
  de fonctions connexes donnée(s).
\end{definition}
...
\item Le système de bord (appellé aussi réseau de bord du fait de son très
grand nombre de lien d'interconnexion), en charge de gérer les composants moteur
\item Le système de communication, en charge de gérer les communications entre
le véhicules et l'extérieur (autres véhicules, le quartier général, etc)
\end{itemize}
...
\begin{definition}
On appelle facteur de forme les dimentions, le type d'alimation, le nomre de
...
\begin{definition}
La disponibilité est la propriété d'un objet à être accessible et utilisable
sur demande d'une entité autorisée.
\end{definition}

\begin{definition}
L'intégrité est une propriété d'un objet a ne pas avoir été modifié par
...
\begin{definition}
La confidentialité est la propriété d'un objet a n'être accessible qu'à des
sujets ayant les droits suffisant pour accéder à l'information qu'il porte.
\end{definition}

\begin{definition}
On appelle domaine de sécurité un système dans lequel l'ensemble des éléments
doivent respecter les même contraintes d'intégrité, de confidentialité et de
disponibilité.
\end{definition}
...
technologies de l'information \cite{cc_docs}. On parle plus régulièrement des Critère Communs
(dénotés CC). Les Critères Communs définissent 7 niveaux, appelés EAL
(Evaluation Assurance Level), de EAL 1 (le plus faible) à EAL 7 (le plus
élevé.

La fragilité d'un système en terme de sécurité dépend entre autre de sa surface d'attaque.
\begin{definition}
On appelle surface d'attaque l'ensemble des chemins permettant d'impacter la
...
\subsubsection{A propos de la sécurité dans les systèmes systroniques}
...
\subsection{Concept liés à la fiabilité et au temps réel}
...
\begin{itemize}
  ...
  \item Le modèle apériodique: Il n'y a pas d'instanciations successives d'une
    tâche, et donc pas de principe d'inter-arrivée.
\end{itemize}
Dans le cadre de ma thèse, les traitements sont induits par des évènements
...
communications, tant en terme de liens de communications locaux/semi-locaux et
...
électrique) et d'exigences de certification fortes (principalement liées à la temps-réel
résistance aux vibrations dans les véhicules), ces derniers ne doivent pas pour
...
\begin{itemize}
  ...
  devant respecter des contraintes temps réel
  \item Le système d'arme, ayant également des propriétés temps réel
  \item Le système de communication, sans propriétés temps réel mais avec un
...
permettre un partage de certains moyens de communications semi-locaux (i.e.
entre des véhicules d'une même patrouille) et distants (entre la patrouille et
les quartiers généraux) entre différents domaines de sécurité. En effet, la multiplication
des antennes génère à la fois un surcout et une problématique de qualité de
...
élevé d'assurance (DO-178B pour le logiciel et DO-254 pour le matériel) assurent
...
systèmes systronique) permettant de respecter à la fois des besoins de sûreté et de
sécurité.
...
éléments matériels positionnées en coupure entre chaque domaine de sécurité et sur la cryptographie.\\
...
d'interconnexion de réseaux, et plusieurs contraintes de sécurité et de sûreté sont considérées afin de répondre à plusieurs exigences précises:
\begin{itemize}
\item Garantie de transfert de données à sens unique
\item Filtrage des données selon une politique de sécurité
\item Maîtrise du profil des flux transférés pour garantir la sûreté du domaine
de sortie
...
définitir la taille des slots temporels du TDM. Cette formalisation est
